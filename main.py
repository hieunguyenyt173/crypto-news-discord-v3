...
# Chỉ là blacklist cơ bản để giảm rác
BLACKLIST_KEYWORDS = [
    "sponsored",
    "advertorial",
    "casino",
...
def detect_mode(now_vn: datetime) -> str:
    # 8h sáng → morning, còn lại mặc định evening
    if now_vn.hour < 14:
        return "morning"
    return "evening"
...
def get_market_snapshot() -> Dict[str, str]:
    ...
    # Không cần key cho mức dùng nhẹ
    result = {
        "btc": "N/A",
        "eth": "N/A",
        "sol": "N/A",
...
def safe_get_source_ref(idx: int, articles: List[Dict[str, Any]]) -> str:
    if isinstance(idx, int) and 1 <= idx <= len(articles):
        return f"[{idx}]"
    return ""


def make_embed_payload(mode: str, ai_data: Dict[str, Any], snapshot: Dict[str, str], now_vn: datetime) -> Dict[str, Any]:
    ...
    for sec in sections[:5]:
        ...
        for item in items[:2]:
            ...
            src = safe_get_source_ref(source_idx, [])
...
def main() -> None:
    ...
    payload = make_embed_payload(mode, ai_data, snapshot, now_vn)
